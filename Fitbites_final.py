...
def meditation_breathing():
    ...
    - Savasana (Corpse Pose) – 5–10 minutes
        ...
        3:"""Day 3: Focus and Clarity
    Focus: Improving focus, concentration, and mental clarity.

    Mountain Pose (Tadasana) – Hold for 1 minute

    Center yourself and focus on your breath.
    Tree Pose (Vrksasana) – Hold for 1 minute per side

    Cultivate balance and mental clarity by focusing your gaze (drishti) on a steady point.
    Warrior II (Virabhadrasana II) – Hold for 1 minute per side

    Ground yourself in the present moment while focusing on your breath.
    Dolphin Pose (Ardha Pincha Mayurasana) – Hold for 1 minute

    A great pose for focus, challenging your mind while keeping it engaged.
    Seated Forward Fold (Paschimottanasana) – Hold for 2 minutes

    Relax and clear your mind, focusing on your breath.
    Nadi Shodhana (Alternate Nostril Breathing) – 5–10 minutes

    This pranayama clears the mind, balances energy, and promotes focus.""",
    4:"""Day 4: Self-Awareness and Introspection
    Focus: Cultivating self-awareness and fostering introspection.

    Child's Pose (Balasana) – Hold for 2 minutes

    Use this time to connect with your breath and let go of any distractions.
    Cobra Pose (Bhujangasana) – Hold for 1 minute

    Open the chest and heart, creating space for self-compassion.
    Bridge Pose (Setu Bandhasana) – Hold for 1–2 minutes

    Strengthen the body and focus on releasing negative emotions.
    Seated Forward Fold (Paschimottanasana) – Hold for 2 minutes

    Tune in with your body and mind while calming the nervous system.
    Seated Meditation – 5–10 minutes

    Practice mindfulness, focusing on the present moment. Reflect on your feelings without judgment.
    Savasana (Corpse Pose) – 5–10 minutes

    Use this time for a deep, reflective meditation. Focus on gratitude for yourself.""",
    6:"""Day 6: Positivity and Gratitude
    Focus: Cultivating a positive mindset and gratitude.

    Mountain Pose (Tadasana) – 1 minute

    Set an intention for the practice: gratitude or positivity.
    Warrior I (Virabhadrasana I) – Hold for 1 minute per side

    Focus on gratitude for your body’s strength and energy.
    Camel Pose (Ustrasana) – Hold for 1–2 minutes

    Open the chest and heart, allowing positive energy to flow freely.
    Cobra Pose (Bhujangasana) – Hold for 1 minute

    Encourage feelings of self-compassion and gratitude.
    Bridge Pose (Setu Bandhasana) – Hold for 1–2 minutes

    Reflect on your positive qualities and accomplishments.
    Meditation for Gratitude – 5–10 minutes

    Sit comfortably and focus on feelings of gratitude for your life and experiences.""",
        7: """
...
def exercises():
    ...
    def main_page():
        clear_frame()
        label = ctk.CTkLabel(app, text="YOUR GOAL\nOUR GOAL", font=("Algerian", 30, "bold"))
        label.pack(pady=40)

        exercises_button = ctk.CTkButton(app, text="I am Ready!!", command=exercises_page, font=("Arial", 16))
        exercises_button.pack(pady=40)

    #  create the exercises category page
    def exercises_page():
        clear_frame()
...
def diet_planner():
    ...
    def open_meal_window():
        def save_diet():
            f=open('diet'+country_code.get()+ '_' + phone_entry.get()+'^'+passwd_entry.get(),'a')
            f.write('Food: '+ food_name_entry.get()+'~'+ 'Category: '+food_category_combo.get()+"~"+ 'Amount in grams: '+serving_entry.get()+"~")
            f.close()
        def recent():
            f=open('diet'+country_code.get()+ '_' + phone_entry.get()+'^'+passwd_entry.get(),'r')
            l=f.read().split("~")
            s=''
            for i in l:
                s=s+i+'\n'
            ...
            a.geometry("900x900")
            a.title("Your meals")
            show_lbl=ctk.CTkLabel(a,text=s,font=("cosmic sans ms",16))
            show_lbl.pack(pady=20)
            a.mainloop()
        ...
        # Frame for the form
        form_frame =ctk.CTkFrame(new_window, fg_color=COLOR_PRIMARY, corner_radius=10)
        form_frame.pack(pady=20, padx=20, fill=BOTH, expand=True)

        # Food Name Entry
        food_name_label = ctk.CTkLabel(form_frame, text="Food Name:", font=("Lucida Sans", 14), text_color=COLOR_TEXT)
        food_name_label.grid(row=0, column=0, padx=10, pady=10, sticky=W)
        food_name_entry = ctk.CTkEntry(form_frame, placeholder_text="Enter food name", fg_color=COLOR_BACKGROUND, text_color=COLOR_TEXT)
        food_name_entry.grid(row=0, column=1, padx=10, pady=10)

        # Food Category Dropdown
        food_category_label = ctk.CTkLabel(form_frame, text="Food Category:", font=("Lucida Sans", 14), text_color=COLOR_TEXT)
        ...
        food_category_combo = ctk.CTkComboBox(form_frame, values=categories, fg_color=COLOR_BACKGROUND, text_color=COLOR_TEXT)
        food_category_combo.grid(row=1, column=1, padx=10, pady=10)

        # Serving Size and Unit
        serving_label = ctk.CTkLabel(form_frame, text="Serving Size:", font=("Lucida Sans", 14), text_color=COLOR_TEXT)
        serving_label.grid(row=2, column=0, padx=10, pady=10, sticky=W)
        serving_frame = ctk.CTkFrame(form_frame, fg_color=COLOR_PRIMARY)
        serving_frame.grid(row=2, column=1, padx=10, pady=10, sticky=W)
        serving_entry = ctk.CTkEntry(serving_frame, width=120, placeholder_text="Amount", fg_color=COLOR_BACKGROUND, text_color=COLOR_TEXT)
        serving_entry.pack(side=LEFT, padx=5)
        unit_options = ["g", "kg", "ml", "cup", "ounce"]
        unit_menu = ctk.CTkOptionMenu(serving_frame, values=unit_options, fg_color=COLOR_BACKGROUND, text_color=COLOR_TEXT)
        unit_menu.pack(side=LEFT, padx=5)

        track_btn=ctk.CTkButton(form_frame, text="Recents",fg_color=COLOR_HIGHLIGHT, hover_color=COLOR_HOVER, command=recent
        )
        track_btn.grid(row=5, column=1,pady=20, sticky=E)
        ...
        save_button = ctk.CTkButton(
            form_frame, text="Save", command=save_diet, fg_color=COLOR_HIGHLIGHT, hover_color=COLOR_HOVER
        )
        save_button.grid(row=4, column=1, pady=20, sticky=E)

        # track_btn=ctk.CTkButton(form_frame, text="Recents",fg_color=COLOR_HIGHLIGHT, hover_color=COLOR_HOVER
        # )
        # track_btn.grid(row=5, column=1,pady=20, sticky=E)


    # Main Calorie Display
    calorie_frame = ctk.CTkFrame(root, fg_color=COLOR_PRIMARY, corner_radius=10)
    calorie_frame.pack(pady=20, padx=20, fill=X)
    calorie_label = ctk.CTkLabel(
        calorie_frame,
        text=f"Eaten: {calories} Cal\nRemaining: {remaining_calories} Cal",
        font=("Lucida Sans", 20),
    ...
    meal_frame = ctk.CTkFrame(root, fg_color=COLOR_BACKGROUND, corner_radius=10)
    meal_frame.pack(pady=10, padx=20, fill=BOTH, expand=True)

    meals = ["Breakfast", "Lunch", "Snacks", "Dinner"]
    for meal in meals:
        meal_section = ctk.CTkFrame(meal_frame, fg_color=COLOR_PRIMARY, corner_radius=10)
        meal_section.pack(pady=10, padx=10, fill=X)
        meal_label = ctk.CTkLabel(meal_section, text=f"   {meal}", font=("Lucida Sans", 18), text_color=COLOR_TEXT)
        meal_label.pack(side=LEFT, padx=20)
        meal_button = ctk.CTkButton(
    ...
    # Run the application
    root.mainloop()  #all vitamins, nutrients, calories, protien, etc planner according to user's workout and meal that one should take before and after workout
    pass
